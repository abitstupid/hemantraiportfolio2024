...
export default function WorkCardSmall(props) {
	const { posts, numToShow, sideProjects } = props;
	const slicedPosts = posts.slice(0, numToShow || undefined);

	return (
		<>
			<article
				className={`${styles.workCardSmallWrapper}  paddingBodyInline marginBottomSectionMedium`}
			>
				{slicedPosts.map((item, idx) => {
					return (
...
WorkCardSmall.propTypes = {
	posts: PropTypes.any,
	numToShow: PropTypes.any,
	sideProjects: PropTypes.any,
};
